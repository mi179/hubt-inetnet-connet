...
> 监控界面顶部实时显示网络状态，按 `B` 返回主菜单，按 `Q` 退出程序。
...
| `cyber-lobster watch` | 断网自动重连 | 7x24 守护 + 实时状态 + `B` 返回 + `Q` 退出 |
...
| 选项 | 快捷键 | 功能 |
|------|--------|------|
| `[1] 🚀 启动守护` | 按 `1` + 回车 | 连网 + 进入 7x24 监控，实时状态栏 + B 返回 + Q 退出 |
