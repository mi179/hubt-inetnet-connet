...
| 特性 | 说明 |
|------|------|
| 🦞 **ANSI Shadow 硬核 Logo** | 块状字体 + Slogan，极简命令行美学 |
| 🔄 **多账号一键切换** | 菜单 [2] 切换 / [3] 添加 / [4] 注销 |
| 🔌 **标准注销下线** | 调用 ePortal 标准 logout 接口，干净下线 |
| 📡 **7x24 断网重连** | 实时状态栏 + B 返回 / Q 退出 / Ctrl+C 返回菜单 |
...
> 监控界面顶部实时显示网络状态，按 `B` 返回主菜单，按 `Q` 退出程序，按 `Ctrl+C` 也可返回主菜单。
...
| `cyber-lobster watch` | 断网自动重连 | 7x24 守护 + 实时状态 + `B` 返回 + `Q` 退出 + `Ctrl+C` 返回 |
| `cyber-lobster setup` | 配置向导 | 添加新账号，验证后保存 |
| `cyber-lobster switch` | 切换账号 | 列出所有已保存账号，选号切换 |
| `cyber-lobster logout` | 注销下线 | 向 ePortal 发送注销请求 |
| `cyber-lobster autostart` | 开机自启 | Windows Startup 脚本 / Linux crontab |
...
| 选项 | 快捷键 | 功能 |
|------|--------|------|
| `[1] 🚀 启动守护` | 按 `1` + 回车 | 连网 + 进入 7x24 监控，实时状态栏 + B 返回 / Q 退出 / Ctrl+C 返回 |
| `[2] 🔄 切换账号` | 按 `2` + 回车 | 列出已保存学号，选号切换（自动下线旧号 + 上线新号） |
| `[3] ➕ 添加账号` | 按 `3` + 回车 | 配置向导，运营商/学号/密码，验证后保存 |
| `[4] 🔌 安全下线` | 按 `4` + 回车 | 发送 ePortal 注销请求 |
...
搭配 `✦ HUBT Network Guardian · 7x24 Zero Drop ✦` Slogan，极简命令行美学。
...
### v0.7.0 (2026-05-17)
- 🆕 监控模式实时状态栏（顶部显示在线/断连 + 时间戳）
- 🆕 B 键返回主菜单 / Q 键退出程序
- 🔧 Ctrl+C 退出 watch 返回主菜单，不再退程序
